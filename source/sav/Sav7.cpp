...
namespace pksm
{
    ...
    void Sav7::mysteryGift(const WCX& wc, int& pos)
    {
        if (wc.generation() == Generation::SEVEN)
        {
            ...
            u8* card = &data[WondercardData + WC7::length * pos];
            std::copy(wc.rawData(), wc.rawData() + WC7::length, card);
            ...
            static constexpr size_t FLAGS_OFFSET = 0x52;
            static constexpr u8 USED_FLAG        = 0x2;
            card[FLAGS_OFFSET]                  &= u8(~USED_FLAG);
