...
namespace pksm
{
    ...
    public:
        static constexpr u16 length = 0x2C8;

        explicit WC9(u8* dt);

        [[nodiscard]] std::string_view extension(void) const override { return ".wc9"; }
        ...
        [[nodiscard]] u8 teraType(void) const;
        [[nodiscard]] s16 heightValue(void) const;
        [[nodiscard]] s16 weightValue(void) const;
        [[nodiscard]] u16 scale(void) const;
        [[nodiscard]] u8 ribbonAtIndex(int index) const;
        ...
        [[nodiscard]] Language language(void) const override { return Language::UNUSED; }
